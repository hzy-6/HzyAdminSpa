...
public class SysDictionaryService : AdminBaseService<SysDictionaryRepository>
{
    ...
    public async Task<List<SysDictionaryDto>> GetDictionaryTreeAsync()
    {
        ...
        return this.CreateDictionaryTree(Guid.Empty, allDictionary);
    }
    ...
    /// </summary>
    /// <returns></returns>
    private List<SysDictionaryDto> CreateDictionaryTree(Guid id, List<SysDictionary> allDictionary)
    {
        ...
        {
            data = allDictionary
                .Where(w => w.ParentId == null || w.ParentId == Guid.Empty)
                .ToList()
                ;
        }
        else
